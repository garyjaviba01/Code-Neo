...
//SELECT count(DISTINCT(propuestas.id)),departamento.nombre FROM propuestas,convocatoria,ciudad,departamento,region WHERE propuestas.id_convocatoria =convocatoria.id and propuestas.ciiudad=ciudad.codigo and ciudad.departamento= departamento.codigo and departamento.region=region.codigo and region.codigo=$_REQUEST[region]  group by departamento.nombre		    
        ...
        $values[$i][2]=0;
        $values[$i][3]=0;
        $estado[$i][0]="GRAN EMPRESA";
        $estado[$i][1]="MEDIANA EMPRESA";
        $estado[$i][2]="MICROEMPRESA";
        $estado[$i][3]="PEQUEÑA EMPRESA";
        while($datos=mysqli_fetch_array($query))
        {
        
        if($datos[1]=="Gran Empresa")
        {
        $values[$i][0]=$datos[0];
        $estado[$i][0]=strtoupper($datos[1]);
            
        }
        else if($datos[1]=="Mediana Empresa")
        {
        $values[$i][1]=$datos[0];
        $estado[$i][1]=strtoupper($datos[1]);
            
        }
        else if($datos[1]=="Microempresa")
        {
        $values[$i][2]=$datos[0];
        $estado[$i][2]=strtoupper($datos[1]);
            
        }
        else{
        $values[$i][3]=$datos[0];
        $estado[$i][3]=strtoupper($datos[1]);
        }
...
<body>
  ...
  <br />
        ...
        <div id="data" style="width: 100%;">
         <table class='table table-bordered' style='width:100%;'><tr><td bgcolor='#EFE0FC' align='center'><b>Región</b></td><td bgcolor='#EFE0FC' align='center'><b>Gran</b></td><td bgcolor='#EFE0FC' align='center'><b>Mediana</b></td><td bgcolor='#EFE0FC' align='center'><b>Micro</b></td><td bgcolor='#EFE0FC' align='center'><b>Pequeña</b></td><td bgcolor='#EFE0FC' align='center'><b>Total región</b></td></tr>  
         <?php 	
         for($i=0;$i<7;$i++){
           ?>  
            
            <tr style='background-color:<?php echo $color_html[$i];?>'><td style="text-align:left;"><?php echo $regiones[$i];?></td><td ><?php echo $values[$i][0];?></td><td ><?php echo $values[$i][1];?></td><td ><?php echo $values[$i][2];?></td><td ><?php echo $values[$i][3];?></td><td ><?php echo array_sum($values[$i]);?></td></tr>
	...
	<script>
	 var total=<?php echo $total;?>;
		...
		var horizontalBarChartData = {
			labels: ['GRAN EMPRESA', 'MEDIANA EMPRESA', 'MICROEMPRESA','PEQUEÑA EMPRESA'],
			datasets: [{
				label: 'AMAZONAS',
