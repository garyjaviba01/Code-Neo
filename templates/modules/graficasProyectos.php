...
if(isset($_COOKIE['user_code']) && isset($_COOKIE['user_rol']) && ($_COOKIE['user_rol']==5 || $_COOKIE['user_rol']==7 || $_COOKIE['user_rol']==8)){
      ...
      // $conect= mysqli_connect("localhost","cpcorien_us_neo","neo2020_","cpcorien_neo_bd");
        ...
        $query=mysqli_query($conect,"SELECT count(DISTINCT(propuestas.id)),region.nombre FROM propuestas,convocatoria,ciudad,departamento,region WHERE propuestas.id_convocatoria =convocatoria.id and propuestas.ciiudad=ciudad.codigo and ciudad.departamento= departamento.codigo and departamento.region=region.codigo  group by region.nombre");
        $totalReg=0;
        while($datos=mysqli_fetch_array($query))
        {
            $totalReg+=$datos[0];
        }
        $query2=mysqli_query($conect,"SELECT count(DISTINCT(propuestas.id)),region.nombre FROM propuestas,convocatoria,ciudad,departamento,region WHERE propuestas.id_convocatoria =convocatoria.id and propuestas.ciiudad=ciudad.codigo and ciudad.departamento= departamento.codigo and departamento.region=region.codigo  group by region.nombre");
        
        function calcularPorcentaje($total,$valor){
            return round($valor*100/$total,0);
        }
        ...
        while($datos=mysqli_fetch_array($query2))
        {
         ...
         $data.=",".$datos[0]."
         ";
         $labels.=",'".strtoupper($datos[1])."  ".calcularPorcentaje($totalReg,$datos[0])."% '";
          $labels2.=",'".strtoupper($datos[1])."'";
         $total+=$datos[0]; 
         } 
         else{
         $data.=$datos[0]; 
...
<html>
    ...
    <body>
        ...
        <script>
            ...
            var html="<table class='table table-bordered' style='width:100%;'><tr><td bgcolor='#EFE0FC' align='center'><b>Región</b></td><td bgcolor='#EFE0FC' align='center'><b>N. Proyectos</b></td><td bgcolor='#EFE0FC' align='center'><b>Porcentaje</b></td></tr>";
                  ...
                  var etiquetas2=[<?php echo $labels2;?>];
                  var cantidades=[];
                  for(var i=0;i<registros;i++)
                  {
                  cantidades[i]=Math.round(datos[i]*100/total)
                  html+="<tr style='background-color:"+color_html[i]+";'><td >"+etiquetas2[i]+"</td><td align='right'>"+datos[i]+"</td><td align='right'>"+cantidades[i]+" %</td></tr>";
                  }
